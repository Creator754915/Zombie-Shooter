...
class MenuButton(Button):
    def __init__(self, text='', **kwargs):
        super().__init__(text, scale=(.25, .075), origin=(0, 0), highlight_color=color.azure,
                         font='assets/fonts/Audiowibe/Audiowibe.ttf', **kwargs)
        self.alpha = 1
...
def start_multiplayer():
    print_on_screen('Comming soon...')


# load menu content
MenuButton(parent=load_menu, text=f'History', y=-1 * button_spacing,
           on_click=start_game)
MenuButton(parent=load_menu, text=f'Dead Game', y=-2 * button_spacing,
           on_click=start_game)
MenuButton(parent=load_menu, text=f'Multiplayer', y=-3 * button_spacing,
           on_click=start_multiplayer)
...
volume_slider = Slider(0, 1, default=Audio.volume_multiplier, step=.1, text='Master Volume:', parent=options_menu,
                       x=-.25)

bg_music = Audio(
    'assets/sounds/music3.mp3',
    loop=True,
    autoplay=True,
    volume=1
)
...
def low():
    if not window.size == (720, 480):
        window.size = (720, 480)
    else:
        print_on_screen("The screen size are already low", scale=2)


def medium():
    if not window.size == (1280, 720):
        window.size = (1280, 720)
    else:
        print_on_screen("The screen size are already medium", scale=2)


def high():
    if not window.size == (1920, 1080):
        window.size = (1920, 1080)
    else:
        print_on_screen("The screen size are already high", scale=2)
...
options_back = MenuButton(parent=options_menu, text='Back', x=-.25, origin_y=3,
...
for menu in (main_menu, load_menu, options_menu):
    def animate_in_menu(menu=menu):
        for i, e in enumerate(menu.children):
            ...
            e.alpha = 0.8
            e.animate('alpha', 0.8, delay=i * .05, duration=.1, curve=curve.out_quad)

            if hasattr(e, 'text_entity'):
                e.text_entity.alpha = 0.8
                e.text_entity.animate('alpha', 1, delay=i * .05, duration=.1)


    menu.on_enable = animate_in_menu

background = Entity(parent=menu_parent, model='quad', texture='shore', scale=(camera.aspect_ratio, 1),
                    color=color.white, z=1, world_y=0)
